/*
 * Copyright © 2015, Anand_Godbole "andy" <aanand.godbole17@gmail.com>
 *
...
#include "andy_charger_control.h"
...
static ssize_t master_switch_store(struct kobject *kobj, struct kobj_attribute *attr, const char *buf, size_t count)
{
	...
	if(rc) {
		...
		pr_info ("andy_charger_control: Invalid switch selection. Using default value %d\n", master_switch);
	} else {
		master_switch = val;
		pr_info ("andy_charger_control: Switch - %d\n", master_switch);
	}
...
static ssize_t custom_current_store(struct kobject *kobj, struct kobj_attribute *attr, const char *buf, size_t count)
{
	...
	if (rc) {
		...
		pr_info ("andy_charger_control: Unsafe current input, switching to defalt value of %d\n", custom_current);
	} else {
...
static int charger_control_probe(void)
{
	...
	charger_control_kobj = kobject_create_and_add("andy_charger_control", kernel_kobj);
